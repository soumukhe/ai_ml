...
def process_documents(progress_bar=None):
    ...
    try:
        ...
        # Configure text splitter with appropriate chunks for gte-large
        text_splitter = RecursiveCharacterTextSplitter.from_tiktoken_encoder(
            encoding_name='cl100k_base',
            chunk_size=450,  # Reduced to stay within embedding model limits
        ...
        for ext, loader in loaders.items():
            try:
                ...
                logger.info("Sample of first chunk content: " + chunks[0].page_content[:200] if chunks else "No chunks")
                ...
                with st.status("Thinking...", expanded=True) as status:
                    try:
                        ...
                        if st.session_state.vector_store is None:
                            status.write("Loading vector store...")
                            embedding_model = HuggingFaceEmbeddings(model_name='thenlper/gte-large')
                            st.session_state.vector_store = Chroma(
                                persist_directory=PERSIST_DIRECTORY,
                                embedding_function=embedding_model
