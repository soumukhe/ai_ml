...
for text in tqdm(data_center_networking['Reason_W_AddDetails'], desc="Analyzing request frequency"):
    try:
...
data_center_networking['RequestFeatureImportance'] = request_importance
data_center_networking['Sentiment'] = sentiment_values

# Remove old Request_Frequency column if it exists
if 'Request_Frequency' in data_center_networking.columns:
    data_center_networking = data_center_networking.drop('Request_Frequency', axis=1)

# Display sample of results
print("\nSample of results with duplicates, request importance, and sentiment:")
print(data_center_networking[['Reason_W_AddDetails', 'Duplicates', 'RequestFeatureImportance', 'Sentiment']].head())
