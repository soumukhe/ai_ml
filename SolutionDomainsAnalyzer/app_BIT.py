...
def run_fuzzy_search_query(query, df, llm):
    ...
    try:
        ...
        # Extract identifiers before passing to agent
        if 'Reason_W_AddDetails' in df_simple.columns:
            df_simple['AHA_ID'] = df_simple['Reason_W_AddDetails'].apply(extract_aha_identifiers)
            df_simple['TAC_Case_ID'] = df_simple['Reason_W_AddDetails'].apply(extract_csc_identifiers)
            df_simple['Service_Now_ID'] = df_simple['Reason_W_AddDetails'].apply(extract_service_now_identifiers)
            df_simple['CAP_Case'] = df_simple['Reason_W_AddDetails'].apply(extract_cap_case_identifiers)
...
5. For AHA_ID, TAC_Case_ID, Service_Now_ID, or CAP_Case:
   # For rows with any AHA_ID:
   df[df['AHA_ID'].str.len() > 0]['row_number'].tolist()
   # For rows with specific AHA_ID (can be CN-I or CSE-I format):
   df[df['AHA_ID'].str.contains('CN-I-12345', case=False, na=False)]['row_number'].tolist()
   df[df['AHA_ID'].str.contains('CSE-I-12345', case=False, na=False)]['row_number'].tolist()
   # For rows with any TAC_Case_ID:
   df[df['TAC_Case_ID'].str.len() > 0]['row_number'].tolist()
   # For rows with any Service_Now_ID:
   df[df['Service_Now_ID'].str.len() > 0]['row_number'].tolist()
   # For rows with any CAP_Case:
   df[df['CAP_Case'].str.len() > 0]['row_number'].tolist()
   # For rows with specific CAP_Case:
   df[df['CAP_Case'].str.contains('CAP12345', case=False, na=False)]['row_number'].tolist()
...
13. For AHA_ID, TAC_Case_ID, Service_Now_ID, and CAP_Case columns:
    - Use str.len() > 0 to find rows with any value
    - Use str.contains() for specific ID searches
    - AHA_ID can be in CN-I-#### or CSE-I-#### format
    - CAP_Case starts with 'CAP' followed by numbers"""
        ...
        if matches:
            try:
                ...
                if isinstance(row_numbers, list) and row_numbers:
                    ...
                    # Add the ID columns to the result dataframe
                    if 'Reason_W_AddDetails' in result_df.columns:
                        result_df['AHA_ID'] = result_df['Reason_W_AddDetails'].apply(extract_aha_identifiers)
                        result_df['TAC_Case_ID'] = result_df['Reason_W_AddDetails'].apply(extract_csc_identifiers)
                        result_df['Service_Now_ID'] = result_df['Reason_W_AddDetails'].apply(extract_service_now_identifiers)
                        result_df['CAP_Case'] = result_df['Reason_W_AddDetails'].apply(extract_cap_case_identifiers)
...
def extract_domain_texts(df, domain):
    ...
    for _, row in domain_df.iterrows():
        ...
        if cleaned_text:  # Only add non-empty texts
            ...
            customer_name = account_name.split(' - ')[-1] if ' - ' in account_name else account_name
            
            # Extract identifiers
            aha_id = extract_aha_identifiers(row['Reason_W_AddDetails'])
            tac_case_id = extract_csc_identifiers(row['Reason_W_AddDetails'])
            service_now_id = extract_service_now_identifiers(row['Reason_W_AddDetails'])
            cap_case = extract_cap_case_identifiers(row['Reason_W_AddDetails'])
            
            # Build metadata string
            metadata = f"Created: {created_date}, Account: {account_name}, **Product**: {product}, Customer: {customer_name}"
            
            # Add identifiers if they exist
            if aha_id:
                metadata += f", **AHA_ID**: {aha_id}"
            if tac_case_id:
                metadata += f", **TAC_Case_ID**: {tac_case_id}"
            if service_now_id:
                metadata += f", **Service_Now_ID**: {service_now_id}"
            if cap_case:
                metadata += f", **CAP_Case**: {cap_case}"
            
            formatted_text = f"{cleaned_text} [METADATA]{metadata}[/METADATA]"
...
def analyze_themes_with_llm(texts, domain, top_n, llm, embedding_model=None):
    ...
    print(f"\n[THEME ANALYSIS] Starting analysis for domain: {domain}, top_n: {top_n}, texts: {len(texts)}")
    start_time = time.time()
    
    # Limit number of examples to prevent token overflow - back to original 150
    max_texts = 150
    total_texts = len(texts)
    
    # If we have more texts than the limit, take a representative sample
    if total_texts > max_texts:
        # Take first 40% and last 40% to get a good distribution
        first_portion = int(max_texts * 0.4)
        last_portion = max_texts - first_portion
        selected_texts = texts[:first_portion] + texts[-last_portion:]
        text_note = f"\n\nNote: Analysis based on a representative sample of {max_texts} entries from {total_texts} total entries."
        print(f"[THEME ANALYSIS] Limiting texts from {total_texts} to {max_texts} for token management")
    else:
    ...
    if embedding_model:
        print(f"[THEME ANALYSIS] Starting feature request grouping with embedding model")
        ...
        print(f"[THEME ANALYSIS] Found {total_feature_requests} potential feature requests")
        
        if feature_texts:
            feature_groups = group_similar_requests(feature_texts, embedding_model, domain)
            print(f"[THEME ANALYSIS] Grouped into {len(feature_groups)} feature request groups")
            ...
            print(f"[THEME ANALYSIS] {len(ungrouped_texts)} texts remain ungrouped")
        
        print(f"[THEME ANALYSIS] Feature grouping completed in {time.time() - group_start_time:.2f} seconds")

    # Prepare feature groups section if we have groups - limit examples to reduce token usage
    if feature_groups:
        print(f"[THEME ANALYSIS] Preparing feature groups section for prompt")
        ...
        feature_groups_section += "   - If group has 5 or more requests: show exactly 5 representative examples\n"
        feature_groups_section += "   - If group has fewer than 5 requests: show all requests in the group\n"
        
        # Add the actual groups data - limit to top 10 groups to reduce token usage
        top_groups = feature_groups[:min(10, len(feature_groups))]
        ...
        for i, group in enumerate(top_groups, 1):
            ...
            # Limit to 5 examples per group (increased from 3)
            for example in group['examples'][:5]:
                # Extract customer name from metadata if present
                ...
                if metadata_start != -1 and metadata_end != -1:
                    ...
                    customer_match = re.search(r'Customer:\s*([^,]+)', metadata)
                    customer_name = customer_match.group(1).strip() if customer_match else "Unknown Customer"
                    
                    # Remove customer from metadata since we're moving it to the front
                    metadata = re.sub(r'Customer:\s*[^,]+,?\s*', '', metadata)
                    
                    # Format the example with customer name at the beginning
                    feature_groups_section += f"- **Customer: {customer_name}** - {text} ({metadata})\n"
                else:
                    feature_groups_section += f"- {example}\n"
        
        # Add ungrouped items section if there are any - limit to 5 examples
        if ungrouped_texts:
            ...
            # Show up to 8 examples with metadata (increased from 5)
    ...
    print(f"[THEME ANALYSIS] Building prompt for LLM")
...
7. For Feature Enhancement Requests theme:
   ...
   - ALWAYS show ALL 5 examples for each group that has 5 or more requests
8. ALWAYS use the actual customer name from the metadata instead of "[Customer]"
9. Include AHA_ID, TAC_Case_ID, Service_Now_ID, and CAP_Case in the examples if they exist in the metadata
10. ALWAYS include the Product field in bold (**Product: [Product Name]**) at the end of each example if it exists in the metadata
11. NEVER abbreviate or truncate the examples - show them in full with all metadata
12. For each theme, include at least 5 examples if available
13. Format each example as: "- **Customer: [Customer Name]** - [Example text] ([Other metadata]) **Product: [Product Name]**"
14. NEVER include the tags [METADATA] or [/METADATA] in your output - extract the information but don't include these tags

For each theme, provide:
1. Clear title and occurrence count
2. Detailed description paragraph (2-3 sentences)
3. Examples from the dataset (at least 5 per theme if available)
4. Impact analysis
5. Recommendations{feature_groups_section}
...
### **Examples:**  
[Examples with customer names and details]
...
{selected_texts}{text_note}"""
    ...
    print(f"[THEME ANALYSIS] Prompt built in {time.time() - prompt_start_time:.2f} seconds")
    print(f"[THEME ANALYSIS] Prompt length: {len(prompt)} characters")
    
    # Invoke the LLM with improved retry logic
    max_retries = 3
    
    # Create a container for retry messages that won't be part of the final report
    retry_container = None
    
    print(f"[THEME ANALYSIS] Starting LLM invocation with max_retries={max_retries}")
    llm_start_time = time.time()
    
    # Define completion markers to check for complete responses
    completion_markers = [
        "Implementation Guidance",
        "Success Metrics",
    ...
    for attempt in range(max_retries):
        try:
            print(f"[THEME ANALYSIS] Attempt {attempt + 1}/{max_retries} started at {time.strftime('%H:%M:%S')}")
            attempt_start_time = time.time()
            
            # Only create the retry container if we're on a retry attempt
            if attempt > 0 and retry_container is None:
                retry_container = st.empty()
                retry_container.warning(f"Incomplete response detected. Retrying... (Attempt {attempt + 1}/{max_retries})")
                print(f"[THEME ANALYSIS] Created retry container for attempt {attempt + 1}")
            
            # Add timeout to prevent hanging on API calls
            print(f"[THEME ANALYSIS] Calling LLM.invoke() with timeout handling")
            
            # Use a try/except with a timeout context if possible
            try:
                response = llm.invoke(prompt)
                print(f"[THEME ANALYSIS] LLM response received in {time.time() - attempt_start_time:.2f} seconds")
                
                # Check if response is an AIMessage object and extract content
                if hasattr(response, 'content'):
                    response_content = response.content
                    print(f"[THEME ANALYSIS] Response length: {len(response_content)} characters")
                else:
                    response_content = str(response)
                    print(f"[THEME ANALYSIS] Response converted to string, length: {len(response_content)} characters")
            except Exception as timeout_error:
                print(f"[THEME ANALYSIS] Possible timeout or API error: {str(timeout_error)}")
                if attempt < max_retries - 1:
                    print(f"[THEME ANALYSIS] Will retry after timeout")
                    continue
                else:
                    raise timeout_error
            
            # Check if the response is complete by looking for multiple markers
            is_complete = True
            missing_markers = []
            ...
            print(f"[THEME ANALYSIS] Response completeness check: {is_complete}")
            if not is_complete:
                print(f"[THEME ANALYSIS] Missing markers: {', '.join(missing_markers)}")
            
            if not is_complete:
                if attempt < max_retries - 1:
                    print(f"[THEME ANALYSIS] Incomplete response detected, will retry")
                    if retry_container is None:
                        retry_container = st.empty()
                    retry_container.warning(f"Incomplete response detected. Retrying... (Attempt {attempt + 1}/{max_retries})")
                    ...
                    print(f"[THEME ANALYSIS] Final attempt produced incomplete response")
                    # Clear the retry container before showing the final error
                    if retry_container:
                        retry_container.empty()
                    return "Failed to generate a complete report after multiple attempts. Please try reducing the number of themes or using a smaller dataset."
            
            # Clear the retry container if we got a complete response
            if retry_container:
                print(f"[THEME ANALYSIS] Clearing retry container after successful response")
                retry_container.empty()
            
            print(f"[THEME ANALYSIS] Complete response received after {attempt + 1} attempts in {time.time() - llm_start_time:.2f} seconds")
            print(f"[THEME ANALYSIS] Total analysis time: {time.time() - start_time:.2f} seconds")
            return response_content
            
        except Exception as e:
            error_message = str(e)
            print(f"[THEME ANALYSIS] Error in attempt {attempt + 1}: {error_message}")
            
            if "maximum context length" in error_message.lower():
                # If we hit context length issues, reduce the number of texts more aggressively
                texts = texts[:len(texts)//3]  # More aggressive reduction
                print(f"[THEME ANALYSIS] Reduced input size due to context length limitations. New size: {len(texts)} texts")
                if retry_container is None:
                    retry_container = st.empty()
                retry_container.warning(f"Reduced input size due to context length limitations. Retrying with {len(texts)} texts.")
            elif "rate limit" in error_message.lower() or "too many requests" in error_message.lower():
                # Handle rate limiting with exponential backoff
                wait_time = (2 ** attempt) + random.random()
                print(f"[THEME ANALYSIS] Rate limit hit. Waiting {wait_time:.2f} seconds before retry")
                if retry_container is None:
                    retry_container = st.empty()
                retry_container.warning(f"API rate limit reached. Waiting {wait_time:.2f} seconds before retry...")
                time.sleep(wait_time)
            elif attempt < max_retries - 1:
                print(f"[THEME ANALYSIS] Will retry after error")
                if retry_container is None:
                    retry_container = st.empty()
                retry_container.warning(f"Error during theme analysis: {error_message}. Retrying... (Attempt {attempt + 1}/{max_retries})")
                # Add a small delay before retrying
                time.sleep(1)
            else:
                print(f"[THEME ANALYSIS] Final attempt failed with error")
                # Clear the retry container before showing the final error
                if retry_container:
                    retry_container.empty()
                return f"Error analyzing themes: {error_message}. Please try reducing the number of themes or using a smaller dataset."
    
    # Clear the retry container if we're exiting the loop
    if retry_container:
        retry_container.empty()
    
    print(f"[THEME ANALYSIS] All attempts failed. Giving up after {time.time() - start_time:.2f} seconds")
    return "Failed to generate theme analysis after multiple attempts. Please try reducing the number of themes or using a smaller dataset."
...
def format_report_for_display(report_text):
    ...
    if report_text.startswith("Error") or report_text.startswith("Failed"):
        return report_text
    
    # Clean up any potential formatting issues
    # Replace any triple or more line breaks with double line breaks
    ...
    # Fix any markdown table formatting
    report_text = re.sub(r'\|\s*\n\s*\|', r'|\n|', report_text)
    
    # Remove visible [METADATA] tags and reformat examples to show customer name at the beginning
    def reformat_example(match):
        full_text = match.group(0)
        text_before_metadata = match.group(1).strip()
        
        # Handle both cases: with closing tag and without
        if len(match.groups()) > 1 and match.group(2):
            metadata_content = match.group(2).strip()
        else:
            metadata_content = ""
        
        # Extract customer name from metadata if available
        customer_match = re.search(r'Customer:\s*([^,]+)', metadata_content)
        customer_name = customer_match.group(1).strip() if customer_match else "Unknown Customer"
        
        # Extract product from metadata if available
        product_match = re.search(r'Product:\s*([^,]+)', metadata_content)
        product_name = product_match.group(1).strip() if product_match else ""
        
        # Format identifiers in bold
        formatted_metadata = metadata_content
        identifiers = ['AHA_ID', 'TAC_Case_ID', 'Service_Now_ID', 'CAP_Case', 'Product']
        for identifier in identifiers:
            formatted_metadata = re.sub(
                rf'{identifier}:\s*([^,]+)', 
                f'**{identifier}**: \\1', 
                formatted_metadata
            )
        
        # Remove the Customer: part from metadata since we're moving it to the front
        formatted_metadata = re.sub(r'Customer:\s*[^,]+,?\s*', '', formatted_metadata)
        
        # Remove the Product: part from metadata since we're moving it to the end
        formatted_metadata = re.sub(r'Product:\s*[^,]+,?\s*', '', formatted_metadata)
        
        # Create the final formatted example
        # If formatted_metadata is empty or just contains whitespace, don't include parentheses
        if not formatted_metadata.strip():
            formatted_example = f"- **Customer: {customer_name}** - {text_before_metadata}"
        else:
            formatted_example = f"- **Customer: {customer_name}** - {text_before_metadata} ({formatted_metadata})"
        
        # Add product at the end if available
        if product_name:
            formatted_example += f" **Product: {product_name}**"
        
        return formatted_example
    
    # First, handle examples with full [METADATA]...[/METADATA] tags
    report_text = re.sub(r'- (.*?)\s*\[METADATA\](.*?)\[/METADATA\]', reformat_example, report_text)
    
    # Then handle examples with just [METADATA] without closing tag
    report_text = re.sub(r'- (.*?)\s*\(\[METADATA\]\)', reformat_example, report_text)
    
    # Handle the case where [METADATA] appears in parentheses at the end of an example
    report_text = re.sub(r'\(\[METADATA\]\)', '', report_text)
    
    # Remove any remaining standalone [METADATA] tags
    report_text = re.sub(r'\[METADATA\]', '', report_text)
    
    # Ensure identifiers are properly displayed in bold for any remaining instances
    identifiers = ['AHA_ID', 'TAC_Case_ID', 'Service_Now_ID', 'CAP_Case', 'Product']
    for identifier in identifiers:
        # Make sure the identifier is in bold and properly formatted
        report_text = re.sub(
            rf'({identifier}):\s*([^,\n]+)', 
            r'**\1**: \2', 
            report_text
    ...
    # Make sure examples are properly formatted with customer names in bold
    report_text = re.sub(
        r'- ([^(]+)\(([^)]+)\)',
        r'- **\1** (\2)',
        report_text
    ...
    print(f"[REPORT FORMAT] Formatted report for display, length: {len(report_text)} characters")
...
def create_pdf_report(report_text, domain):
    """Convert markdown report to PDF using ReportLab with improved formatting and include the pie chart at the top."""
    try:
        import io
        ...
        elements = []
        # --- PIE CHART GENERATION AND INSERTION ---
        # Parse theme distribution from report_text
        ...
        while i < len(lines):
            ...
            if m:
                ...
                for j in range(1, 6):
                    if i + j < len(lines):
                        occ = re.match(r'^\*\*Occurrences\*\*:\s*(\d+)', lines[i + j])
                        if occ:
                            titles.append(title)
                            counts.append(int(occ.group(1)))
                            break
                i += 1
            else:
                i += 1
        if titles and counts and sum(counts) > 0:
            max_idx = counts.index(max(counts))
            pull = [0.1 if idx == max_idx else 0 for idx in range(len(counts))]
            fig = px.pie(
                names=titles,
                values=counts,
            ...
            fig.update_traces(pull=pull, textinfo='label+percent', marker=dict(line=dict(color='#000', width=1)))
            ...
            img = Image(img_buffer, width=450, height=300)  # Adjust size as needed
            elements.append(img)
            elements.append(Spacer(1, 24))
        # --- END PIE CHART INSERTION ---
        # Split into lines
        current_list = []
        ...
        for line in lines:
            ...
            if line.startswith('# '):
                ...
                elements.append(Paragraph(md_bold_to_html(text), styles['CustomHeading1']))
            elif line.startswith('## '):
                if current_list:
                    elements.extend(current_list)
                    current_list = []
                text = line[3:].strip('*').strip()
                elements.append(Paragraph(md_bold_to_html(text), styles['CustomHeading2']))
            elif line.startswith('### '):
                if current_list:
                    elements.extend(current_list)
                    current_list = []
                text = line[4:].strip('*').strip()
                elements.append(Paragraph(md_bold_to_html(text), styles['CustomHeading3']))
            # Bullet points
                ...
                if '**' in text and ' – ' in text:
                    ...
                    if len(parts) == 2:
                        ...
                        formatted_text = f"• <b>{customer}</b> – {md_bold_to_html(details)}"
                    ...
                    formatted_text = f"• <b>{category}</b>: {md_bold_to_html(details)}"
                    current_list.append(Paragraph(formatted_text, styles['CustomBullet']))
                else:
                    current_list.append(Paragraph(f"• {md_bold_to_html(text)}", styles['CustomBullet']))
            else:
                if current_list:
                    elements.extend(current_list)
                    current_list = []
                elements.append(Paragraph(md_bold_to_html(line), styles['CustomBody']))
        if current_list:
            elements.extend(current_list)
        doc.build(elements)
        pdf = buffer.getvalue()
...
def process_uploaded_file(uploaded_file):
    ...
    try:
        ...
        if sheet_name:
            ...
            columns_to_fill = ['Account Name', 'Product', 'Use Case', 'Solution Domain']
...
def extract_csc_identifiers(text):
    """Extract CSC identifiers from text"""
    import re
    if not isinstance(text, str):
        return ""
    
    # Pattern to match CSC followed by letters and numbers
    pattern = r'CSC[a-zA-Z0-9]+'
    matches = re.findall(pattern, text)
    
    # Return unique CSC identifiers joined by comma
    if matches:
        return ", ".join(set(matches))
    return ""

def extract_aha_identifiers(text):
    """Extract AHA identifiers (CN-I and CSE-I) from text"""
    import re
    if not isinstance(text, str):
        return ""
    
    # Pattern to match CN-I or CSE-I followed by numbers
    pattern = r'(?:CN-I-\d+|CSE-I-\d+)'
    matches = re.findall(pattern, text)
    
    # Return unique AHA identifiers joined by comma
    if matches:
        return ", ".join(set(matches))
    return ""

def extract_service_now_identifiers(text):
    """Extract INC identifiers from text"""
    import re
    if not isinstance(text, str):
        return ""
    
    # Pattern to match INC followed by numbers and possibly letters
    pattern = r'INC\d+[a-zA-Z0-9]*'
    matches = re.findall(pattern, text)
    
    # Return unique INC identifiers joined by comma
    if matches:
        return ", ".join(set(matches))
    return ""

def extract_cap_case_identifiers(text):
    """Extract CAP case identifiers from text"""
    import re
    if not isinstance(text, str):
        return ""
    
    # Pattern to match CAP followed by numbers
    pattern = r'CAP\d+[a-zA-Z0-9]*'
    matches = re.findall(pattern, text)
    
    # Return unique CAP case identifiers joined by comma
    if matches:
        return ", ".join(set(matches))
    return ""
...
def display_main_analysis_tab():
    ...
    if st.session_state.processed_df is not None:
        ...
        # Find the index of Reason_W_AddDetails column
        reason_col_idx = df_display.columns.get_loc('Reason_W_AddDetails') if 'Reason_W_AddDetails' in df_display.columns else -1
        
        if reason_col_idx >= 0:
            # Extract CSC identifiers
            df_display['TAC_Case_ID'] = df_display['Reason_W_AddDetails'].apply(extract_csc_identifiers)
            
            # Extract AHA identifiers
            df_display['AHA_ID'] = df_display['Reason_W_AddDetails'].apply(extract_aha_identifiers)
            
            # Extract Service Now identifiers
            df_display['Service_Now_ID'] = df_display['Reason_W_AddDetails'].apply(extract_service_now_identifiers)
            
            # Extract CAP case identifiers
            df_display['CAP_Case'] = df_display['Reason_W_AddDetails'].apply(extract_cap_case_identifiers)
            
            # Reorder columns to place AHA_ID, TAC_Case_ID, Service_Now_ID, and CAP_Case right after Reason_W_AddDetails
            cols = list(df_display.columns)
            cols.remove('TAC_Case_ID')
            cols.remove('AHA_ID')
            cols.remove('Service_Now_ID')
            cols.remove('CAP_Case')
            cols.insert(reason_col_idx + 1, 'AHA_ID')
            cols.insert(reason_col_idx + 2, 'TAC_Case_ID')
            cols.insert(reason_col_idx + 3, 'Service_Now_ID')
            cols.insert(reason_col_idx + 4, 'CAP_Case')
...
def display_fuzzy_search_tab():
    ...
    if st.session_state.processed_df is not None:
        ...
        with st.expander("📝 Example Queries"):
            ...
            - Show me all rows where customer has TAC case
            - Show me all rows where customer has CAP case
            - Show me all rows where customer has CAP case and sentiment is negative
            """)
        ...
        if st.session_state.processed_df is not None:
            ...
            if st.button("Generate Theme Analysis Report", type="primary"):
                try:
                    ...
                    print(f"[REPORTS] Starting report generation for domain: {selected_domain}, themes: {num_themes}")
                    
                    # Set a timeout for report generation
                    max_report_time = 180  # 3 minutes max
                    
                    try:
                        report = analyze_themes_with_llm(domain_texts, selected_domain, num_themes, llm)
                        report_time = time.time() - report_start_time
                        print(f"[REPORTS] Report generation completed in {report_time:.2f} seconds")
                    except Exception as e:
                        print(f"[REPORTS] Error during report generation: {str(e)}")
                        raise e
            ...
            if st.session_state.current_report:
                ...
                # Check if the report is an error message
                if st.session_state.current_report.startswith("Error") or st.session_state.current_report.startswith("Failed"):
                    st.error(st.session_state.current_report)
                else:
                    # Display pie chart of theme distribution
                    display_theme_distribution_pie(st.session_state.current_report)
                    # Display report with proper formatting
                    formatted_report = format_report_for_display(st.session_state.current_report)
...
def display_theme_distribution_pie(report_text):
    """Display a pie chart of theme distribution, highlighting the most frequent theme."""
    import re
    if not report_text:
        st.info("No theme distribution data available for pie chart.")
        return
    lines = [line.strip() for line in report_text.splitlines()]
    titles = []
    counts = []
    i = 0
    while i < len(lines):
        # Match lines like: ## **1. Theme Title**
        m = re.match(r'^##\s*\*\*(\d+(?:\.\d+)*)\.\s*(.+?)\*\*', lines[i])
        if m:
            title = m.group(2).strip()
            # Look ahead up to 5 lines for '**Occurrences**: N'
            for j in range(1, 6):
                if i + j < len(lines):
                    occ = re.match(r'^\*\*Occurrences\*\*:\s*(\d+)', lines[i + j])
                    if occ:
            ...
            i += 1
    if not titles or not counts or sum(counts) == 0:
        st.info("No theme distribution data available for pie chart.")
        return
    max_idx = counts.index(max(counts))
    pull = [0.1 if i == max_idx else 0 for i in range(len(counts))]
    import plotly.express as px
    fig = px.pie(
        names=titles,
        values=counts,
