...
def extract_domain_texts(df, domain):
    ...
    for _, row in domain_df.iterrows():
        ...
        if cleaned_text:  # Only add non-empty texts
            ...
            formatted_text = f"{cleaned_text} [METADATA]Created: {created_date}, Account: {account_name}[/METADATA]"
...
def analyze_themes_with_llm(texts, domain, top_n, llm):
    ...
    # Limit the number of texts to analyze to prevent token overflow
    max_texts = 150
    ...
    # Pre-analyze texts to get accurate counts
    feature_request_count = sum(1 for text in selected_texts 
                              if any(keyword in text.lower() 
                                    for keyword in ['feature', 'enhancement', 'request', 'new']))
    
    # Add the counts to the prompt
    counts_section = f"""
EXACT COUNTS FOR ANALYSIS:
• Total Entries Analyzed: {len(selected_texts)}
• Feature Enhancement Requests: {feature_request_count}

You MUST use these exact counts in your analysis. The Feature Enhancement Requests count of {feature_request_count} is final and must be used as the occurrence count for Theme #1.
"""
...
IMPORTANT RULES:
1. You MUST use the exact counts provided above - do not recalculate or modify them
2. Feature Enhancement Requests MUST be Theme #1 with EXACTLY {feature_request_count} occurrences
3. You MUST generate EXACTLY {top_n} distinct themes
4. Each theme must have its own complete analysis
5. Show a maximum of 15 examples per theme
6. Format examples consistently with all metadata
7. Double-check that your counts match the provided statistics
...
**Occurrences**: {feature_request_count}

[1-2 sentence description]

### **Examples:**  
[Show up to 15 examples, formatted as:]
- **[Customer]** – [Details] (Created: [Date], Account: [Name])

[Continue with remaining {top_n-1} themes...]
## **[2-{top_n}]. [Theme Title]**  
**Occurrences**: [EXACT count]
