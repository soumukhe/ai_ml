...
5. For AHA_ID, TAC_Case_ID, or Service_Now_ID:
...
13. For AHA_ID, TAC_Case_ID, and Service_Now_ID columns:
...
9. For each group, show at least 5 examples if there are 5 or more available
10. Include AHA_ID, TAC_Case_ID, and Service_Now_ID in the examples if they exist in the metadata
...
3. Format each ungrouped item as: "- **[Actual Customer Name]** – [Details] (Created: [Date], Account: [Name], **Product**: [Product], **AHA_ID**: [ID if exists], **TAC_Case_ID**: [ID if exists], **Service_Now_ID**: [ID if exists])"
...
- **[Actual Customer Name]** – [Details] (Created: [Date], Account: [Name], **Product**: [Product], **AHA_ID**: [ID if exists], **TAC_Case_ID**: [ID if exists], **Service_Now_ID**: [ID if exists])
...
{selected_texts}{text_note}"""
    ...
    for attempt in range(max_retries):
        try:
            ...
            if "Success Metrics" not in response:
                if attempt < max_retries - 1:
                    st.warning(f"Incomplete response detected. Retrying... (Attempt {attempt + 1}/{max_retries})")
                    continue
                else:
                    st.error("Failed to generate a complete report after multiple attempts.")
            ...
            if "maximum context length" in error_message.lower():
                # If we hit context length issues, reduce the number of texts
                texts = texts[:len(texts)//2]
                st.warning(f"Reduced input size due to context length limitations. Retrying with {len(texts)} texts.")
            elif attempt < max_retries - 1:
                st.warning(f"Error during theme analysis: {error_message}. Retrying... (Attempt {attempt + 1}/{max_retries})")
            else:
                st.error(f"Failed to analyze themes after {max_retries} attempts: {error_message}")
                return f"Error analyzing themes: {error_message}"
    
    return "Failed to generate theme analysis after multiple attempts."
...
def extract_service_now_identifiers(text):
    ...
    pattern = r'(INC\d+)'     # r'INC\d+[a-zA-Z0-9]*'
    matches = re.findall(pattern, text)
...
def display_main_analysis_tab():
    ...
    if st.session_state.processed_df is not None:
        ...
        if reason_col_idx >= 0:
            ...
            # Reorder columns to place AHA_ID, TAC_Case_ID, and Service_Now_ID right after Reason_W_AddDetails
